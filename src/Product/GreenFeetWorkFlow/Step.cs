...
public class Step
{
    ...
    /// <summary>
    /// A key that can be set to later retrieve the step and invoke it using a 'Command'
    /// </summary>
    public string? SearchKey { get; set; }

    /// <summary>
    /// Field is only used for when new steps needs be persisted for the first time.
    /// </summary>
    public object? InitialState { get; set; }

    /// <summary>
    /// the state as it is formatted and persisted in the persistencelayer
    /// </summary>
    public string? PersistedState { get; set; }

    /// <summary>
    /// the name of the formatter used to serialize/deserialize state
    /// </summary>
    ...
    /// <summary>
    /// The earliest point in time the step will be executed
    /// </summary>
    ...
    /// <summary>
    /// typically set in various error situations by the framework, unless you overwrite it
    /// </summary>
    ...
    /// Mark the step as finished successfully.
    /// </summary>
    public ExecutionResult Done(Step newStep) => ExecutionResult.Done(newStep);

    /// <summary>
    /// Mark the step as finished successfully.
    /// </summary>
    public ExecutionResult Done(List<Step> newSteps) => ExecutionResult.Done(newSteps.ToArray());
    ...
    /// Mark the step as finished with failure.
    /// </summary>
    public ExecutionResult Fail(List<Step> newSteps) => ExecutionResult.Fail(newSteps.ToArray());

    /// <summary>
    /// Mark the step as finished with failure.
    /// </summary>
    public ExecutionResult Fail(params Step[] newSteps) => ExecutionResult.Fail(newSteps);
